...
class DCGAN(object):
  def __init__(self, sess, input_height=128, input_width=128, crop=False,
         batch_size=32, sample_num = 32, output_height=128, output_width=128,
         y_dim=None, z_dim=100, gf_dim=64, df_dim=64,
         gfc_dim=1024, dfc_dim=1024, c_dim=1, dataset_name='gan',
    ...
    Args:
      ...
      y_dim: (optional) Dimension of dim for y. [None] #set to 10 for MNIST, None otherwise
    ...
    if self.dataset_name == 'mnist':
      ...
      self.c_dim = 1
      self.data = np.load("data/batch.npy")
      self.data = np.reshape(self.data, [len(self.data),128,128,1])
      '''
      #data_path = os.path.join(self.data_dir, self.dataset_name, self.input_fname_pattern)
      #finds all images in folder data
  ...
  def train(self, config):
    ...
    for epoch in xrange(config.epoch):
      if config.dataset == 'mnist':
        ...
        self.data = np.load("data/batch.npy")
        self.data = np.reshape(self.data, [len(self.data),128,128,1])
        batch_idxs = min(len(self.data) / config.batch_size, config.train_size)
        #print(batch_idxs)
        '''      
      ...
      for idx in xrange(0, int(batch_idxs - 2)):
        if config.dataset == 'mnist':
  ...
  def discriminator(self, image, y=None, reuse=False):
    with tf.variable_scope("discriminator") as scope:
      ...
      if not self.y_dim:
        #df_dim = dimensions of discrim filters in first discrim layer
  ...
  def generator(self, z, y=None):
    with tf.variable_scope("generator") as scope:
      #y_dim is the dimension for y (if a dataset has categories), which is set to 10 for MINST and None otherwise
      #non-categorical data will not have a y_dim
      if not self.y_dim:
        #originally 4 layers (to h4, we added 5,6)
        #get the expected output height and width
        ...
        #project `z`, reshape and pass through ReLu activation
        #takes input z, output size (gf_dim * 8 * s_h16 * s_h16)
        #bc gf_dim * 8 is the last layer of the discriminator
        self.z_, self.h0_w, self.h0_b = linear(
            z, self.gf_dim*32*s_h64*s_w64, 'g_h0_lin', with_w=True)
        #reshapes into [m, s_h16, s_h16, gf_dim * 8]
        self.h0 = tf.reshape(
            self.z_, [-1, s_h64, s_w64, self.gf_dim * 32])
        h0 = tf.nn.relu(self.g_bn0(self.h0))
        #h0 is the output of the first hidden layer

        #Layer 2: Deconv + Relu
        #pass h0 through the second hidden layer, then a relu activation to get h1
